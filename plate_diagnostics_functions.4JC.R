...
#   bc<-list()
#   for(i in 1:4){
#     tc[[i]]<-  read.csv(paste(libraries[i],".coutt.csv.gz", sep=""), header = TRUE, sep = "\t",row.names =1)
#     bc[[i]] <- read.csv(paste(libraries[i],".coutb.csv.gz", sep=""), header = TRUE, sep = "\t",row.names =1)
#     rc[[i]] <- read.csv(paste(libraries[i],".coutc.csv.gz", sep=""), header = TRUE, sep = "\t",row.names =1)
#   }
#   merge.tc<-intersectmatrix(tc[[1]],intersectmatrix(tc[[2]],intersectmatrix(tc[[3]],tc[[4]])))
#   merge.bc<-intersectmatrix(bc[[1]],intersectmatrix(bc[[2]],intersectmatrix(bc[[3]],bc[[4]])))
#   merge.rc<-intersectmatrix(rc[[1]],intersectmatrix(rc[[2]],intersectmatrix(rc[[3]],rc[[4]])))
...
#
#   #Read files
#   files <- list.files(dir, "*.csv.gz")
#   split <- strsplit(files,split = ".(coutb|coutc|coutc).csv.gz")
#   file_names <- unique(as.character(data.frame(split, stringsAsFactors = FALSE)[1,]));   print(11); print(file_names); print(file_names)
#
#   #This check if all necessary files are in the script
#   error <- ""
#   for(i in 1:length(file_names)){
#     print(i)
#     print(file_names[i])
#     if(file.exists(paste(dir, file_names[i],".coutb.csv.gz", sep="")) == FALSE){
#       f <- paste(file_names[i], ".coutb.csv.gz", " is not found!", sep = "")
#       error <- paste(error, "\n", f)
#     }
#
#     if(file.exists(paste(dir, file_names[i],".coutc.csv.gz", sep="")) == FALSE){
#       f <- paste(file_names[i], ".coutc.csv.gz", " is not found!", sep = "")
#       error <- paste(error, "\n", f)
#     }
#
#     if(file.exists(paste(dir,file_names[i],".coutt.csv.gz", sep="")) == FALSE){
#       f <- paste(file_names[i], ".coutt.csv.gz", " is not found!", sep = "")
#       error <- paste(error, "\n", f)
#     }
#   }
#
